---
title: ""
author: "Ken Steif, Ph.D - @Ken Steif"
date: "June, 2021"
output:  
  html_document:
   keep_md: true
...
```{r dl basemaps, message = FALSE, results = FALSE, warning= FALSE, cache=TRUE}
nhoods <- 
...
```{r dl fires, message = FALSE, results = FALSE, warning= FALSE, cache=TRUE}
fires <- 
...
```{r dl land use, message = FALSE, results = FALSE, warning= FALSE, cache=TRUE}
...
```{r viz count vacants, cache=TRUE}
ggplot() +
...
```{r nn dist vacants viz, cache=TRUE}
ggplot() +
...
```{r yrbuilt and sq ft, cache=TRUE}
vars_net <-
...
```{r sig, cache=TRUE, fig.width=8, fig.height=8}
...
```{r run cv, message = FALSE, results = FALSE, cache=TRUE}
...
```{r error by reg and nhood, cache=TRUE}
...
```{r error by reg and nhood map, cache=TRUE}
error_by_reg_and_fold %>%
  ggplot() +
...
```{r map outcomes, cache=TRUE, fig.width=6, fig.height=10}
grid.arrange(ncol=1,    
  reg.summary %>%
    group_by(Regression, cvID) %>%
...
```{r risk factors cats, cache=TRUE}
fire_risk_sf <-
...
```{r map risk jawns, fig.width = 8, fig.height=5,cache=TRUE}
